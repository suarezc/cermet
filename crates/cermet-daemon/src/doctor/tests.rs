...
// ---- an unseeded daemon (zero action templates) surfaces a loud WARN, never a refusal ----

#[test]
fn zero_action_templates_warns_but_still_serves() {
    // An unseeded home (absent actions.d) loads no templates, so every real github/vercel
    // request denies "unsupported action". That is a legitimate fresh state, so doctor must WARN
    // loudly (naming the path + the consequence) — never refuse.
    let (home, runtime) = ok_runtime();
    let report = run(
        home.path(),
        &runtime,
    ...
    let at = find(&report, "action_templates");
    assert_eq!(
        at.status, "warn",
        "zero templates is a loud warning: {at:?}"
    );
    assert!(
        at.detail.contains("actions.d"),
        "the detail names the actions.d path: {at:?}"
    );
    assert!(
        at.detail.contains("DENY") || at.detail.contains("unsupported"),
        "the detail names the consequence (every real request denies): {at:?}"
    );
    assert!(
        report.serving,
        "an unseeded daemon still SERVES — zero capability is not a refusal"
    );
}

#[test]
fn empty_actions_dir_also_warns() {
    // A present-but-empty actions.d (no *.yaml) is the same zero-capability state as an absent one.
    let (home, runtime) = ok_runtime();
    std::fs::create_dir_all(home.path().join("actions.d")).unwrap();
    let report = run(
        home.path(),
        &runtime,
        &runtime,
    ...
    assert_eq!(find(&report, "action_templates").status, "warn");
    // Only a warn — no `fail` — so the service-mode daemon keeps serving.
    assert!(
        report.serving,
        "an empty actions.d is a warn, not a hard refuse"
    );
}

#[test]
fn loaded_action_templates_report_ok() {
    // A seeded actions.d with a live *.yaml template reports ok (no capability warning). A staged
    // (`.yaml.staged`) file is NOT a live template and must not count as capability.
    let (home, runtime) = ok_runtime();
    let actions = home.path().join("actions.d");
    std::fs::create_dir_all(&actions).unwrap();
    std::fs::write(
        actions.join("github.attacker_planted.yaml"),
        "provider: github\n",
    )
    .unwrap();
    std::fs::write(actions.join("github.e2e_probe.yaml.staged"), "staged\n").unwrap();
    let report = run(
        home.path(),
        &runtime,
        &runtime,
    ...
    assert_eq!(
        at.status, "ok",
        "a seeded actions.d has no capability warning: {at:?}"
    );
    assert!(
        at.detail.contains('1'),
        "the ok detail counts only the live *.yaml (not the .staged file): {at:?}"
    );
}
